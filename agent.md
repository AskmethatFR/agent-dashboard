# agent.md — agent contract for `agent-dashboard`
...
>
> This contract is **agent-agnostic**. Paths under `~/.agent-team/` are
> placeholders that each agent runtime maps to its own configuration
> root (for example, a Claude Code runtime maps them under
> `~/.claude/`). See the agent-specific stub at the repo root (e.g.
> `CLAUDE.md`) for the runtime-specific mapping.
...
`~/.agent-team/team-context/TEAM.md` (PM, Architect, DevA, DevB, QA,
Security). The orchestration contract is
`~/.agent-team/team-context/PROTOCOL.md` v2. Read both before claiming a
ticket.
...
- **Commits:** Conventional Commits (`feat`, `fix`, `docs`, `chore`,
  `refactor`, `test`). Each commit message ends with a
  `Co-Authored-By:` trailer identifying the agent that produced it
  (the exact string is configured per agent runtime — see the runtime
...
4. `~/.agent-team/team-context/PROTOCOL.md` — orchestration & state machine
5. `~/.agent-team/team-context/TEAM.md` — who does what
...
Full index: `~/.agent-team/knowledge-base/INDEX.md`.
...
This repository runs an **autonomous GitHub agent**: the agent
workflow under `.github/workflows/` triggers an AI agent on a
self-hosted runner. Three trigger paths:

- `issues:labeled` with the **agent-trigger label** (e.g. `ai-agent`)
  — full pipeline (triage → delegate → test → PR).
- `issue_comment:created` by the repo owner, **on an issue (not a PR)**
  carrying `status:escalated` or the agent-trigger label, **and
  containing the agent invocation marker** (e.g. `/agent`) —
  **escalation handling**: the comment is treated as the human's
  resolution of a previously-raised escalation, the agent re-enters
  the team workflow accordingly. The marker is required so that
  ordinary review comments (especially on PRs) never re-trigger a
  run. **Never include the invocation marker in your own comments**
  — that would trigger an infinite loop.
- `workflow_dispatch` with `issue_number` — manual test path.

> The exact label name (`ai-agent`, `claude-agent`, ...) and marker
> string (`/agent`, `/claude`, ...) depend on the agent runtime
> configured for this repo. See the runtime stub at the repo root.
...
`~/.agent-team/team-context/TEAM.md`, following the protocol in
`~/.agent-team/team-context/PROTOCOL.md` v2.

You do NOT write code, design architecture, or run code reviews
yourself. You delegate to sub-agents via your runtime's sub-agent
mechanism:

- **Project Architect** (`~/.agent-team/agents/project-architect.md`) —
  owns the tech spec and orchestrates devs / QA / security.
- **Developer** (`~/.agent-team/agents/developer.md`) — implements in
  TDD Chicago school, cross-reviews the peer's PR.
- **QA** (`~/.agent-team/agents/qa.md`) — techno-functional verification
  + Definition of Done.
- **Security** (`~/.agent-team/agents/security.md`) — OWASP / secret /
...
knowledge base index is at `~/.agent-team/knowledge-base/INDEX.md`.

### Operating steps — labeled-trigger run

1. **Triage before delegating.** Read this `agent.md`, then
...
issue carrying `status:escalated` or the agent-trigger label, the
comment is the **human's input on a previously-raised question** —
typically the resolution of an escalation.
...
- Never expose `GITHUB_TOKEN`, your runtime's `AGENT_TOKEN`, or any
  other secret in a log, comment, or PR body.
- Always end your commits with the `Co-Authored-By:` trailer
  configured for your agent identity.
