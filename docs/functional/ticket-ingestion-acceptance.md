...
| AC7 | Application startup | The SQLite database file lives under the path resolved from the `DATA_PATH` configuration key. **The default is owned by the composition root: local dev resolves to a writable `data/` folder under the content root (`Path.Combine(ContentRootPath, "data")`); Docker sets `DATA_PATH=/data` explicitly.** The schema is created **idempotently** at startup; the migration approach is documented in [[adr-010]]. |
